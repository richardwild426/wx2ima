...
5. 选择账号，粘贴微信公众号文章链接并提交。网页不再限制粘贴条数，会校验、去重后自动分批加入队列，并显示已确认入队的数量。

链接确认入队后，导入在后台执行，关闭页面不会取消已入队任务；尚未提交完毕时关闭页面，未确认的部分不会自动进入后台。分批提交失败时，输入框保留尚未确认入队的链接，方便继续提交。浏览器会记住所选账号。更改分配规则只影响后续分配，不会移动已经导入的文章。

## 排队与第三方限流

所有账号共用一个持久化导入队列，按提交顺序串行处理文章。提交接口只负责入队，不再因为已有 20 个任务而拒绝新链接。一次请求最多承载 100 条链接且请求体上限为 32 KiB；网页自动按最多 20 条和字节数分块，因此这不是粘贴总数限制。数据库原子写入和唯一约束防止并发重复入队，单个分块要么全部写入，要么全部回滚。

任务完成或失败后自动启动下一条；每分钟的后台调度负责补充唤醒、检查异常退出的任务，并恢复结果不确定的 Workflow 创建请求。恢复时复用已保留的 Workflow ID，避免重复启动。同一文章的重试也先回到队列。

第三方明确返回限流时，保留当前进度并暂停队列，等待 `Retry-After` 指定的时间后继续；没有明确等待时间时采用至少一分钟的保守等待。不会把网络中断或未知错误当成“可以安全重发”的限流，尤其不会盲目再次添加 IMA 条目。网页显示排队总数和处理中的数量，列表优先展示处理中的任务，其余按最近更新时间展示，最多 100 条。

长风与 IMA 未公开具体 QPS、并发数或每日请求配额，因此单并发是本站的保守策略，并非声称第三方只允许一个请求。IMA 文档中的“每次最多 10 个 URL”属于 URL 导入接口，本站使用 PDF 文件上传流程，不受该 URL 条数限制。服务端明确返回的频控信号始终优先。
...
- 页面展示每个账号最近 100 条导入记录，并显示该账号全部排队和处理中的任务数；整个站点同时处理一篇文章，其余留在数据库队列中等待。
